...
class MessageWindow(tk.Canvas):
    def __init__(self, container, *args, **kwargs):
        ...
        self.messages_frame = ttk.Frame(self, style='Messages.TFrame')
    ...
    def _on_mousewheel(self, event):
        self.yview_scroll(-int(event.delta / 120), "units") #120 depending on the OS
    ...
    def _create_message_container(self, message_content, message_time, message_labels):
        ...
        def reconfigure_message_labels(event):
            for label, _ in message_labels:
                label.configure(wraplength=min(container.winfo_width() - 130, MAX_MESSAGE_WIDTH)) #-130 to account for padding
